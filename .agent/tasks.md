...
| # | Задача | Статус | Что сделано |
|---|--------|--------|-------------|
| 1 | Per-page `appHash` | [x] | Все 8 страниц ведут на свой инструмент вместо `studio`: merge/compress/ocr/auto-toc — в мастер, edit/sign — в canvas-редактор, split — в canvas (там теперь кнопка Split), convert — в canvas (workspace `/studio/convert` выкидывает на canvas без открытого документа). Карта целей вынесена в `shared/seo-app-targets.ts`, сайт и тест читают её оттуда |
| 2 | `?upload=1` в мастере | [x] | v6 `WizardShell` понимает параметр (`SmartUploadZone.autoOpen`) — раньше умел только canvas |
