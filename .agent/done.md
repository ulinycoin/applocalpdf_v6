...
### FREE-01: единый дневной лимит — 3 файла на вход и 3 на выход
- Введён `src/app/platform/daily-file-quota.ts`: счётчики `processed` и `downloaded` на сегодня, лимит 3 каждый, только для free-плана, localStorage-ключ `localpdf_daily_files`, автоматический сброс по дате. 5 unit-тестов, включая переход через сутки и отсутствие localStorage (не должно ломать приложение).
- Обработка покрыта во всех точках входа в продукт: canvas (`handleIncomingFiles`), мастер (`wizard-flow-core.handleFilesAdded`), standalone-страница OCR (`handlePickFiles`). Проверка — до записи файлов, списание — после успешной записи, batch больше остатка отклоняется целиком.
- Скачивание покрыто во всех точках выхода: `download-output-files.ts` (мастер и OCR-страница), экспорт из canvas (`studio-top-nav`), convert-workspace (`downloadResults`, `downloadSingleResult`), ZIP компресса, панель Auto-TOC. При превышении — существующий пейвол с текстом «Free includes 3 downloads per day…».
- Удалены прежние поштучные лимиты (`daily-usage.ts`, «3/день на инструмент» для Text/Sign/Whiteout/Protect/Auto-TOC и др.) — теперь одно правило на весь продукт вместо одиннадцати разных, плюс мёртвая ветка `setPaywallReason`.
- Сайт приведён в соответствие (лимит нельзя прятать): `pricing.astro` (описание, FAQ-схема, список фич — число берётся из `FREE_DAILY_FILE_LIMIT`), `index.astro`, две compare-страницы, `pdf-tools-without-upload.astro`, `llm.txt` и `.well-known/llm.txt`, `index-ai.md`.
- Проверено в браузере на локальной сборке: canvas — 3 файла приняты, 4-й даёт пейвол «Free includes 3 files per day and you have added all of them today»; convert-workspace — 3 скачивания прошли, 4-е даёт «…used all of them today»; страница OCR — 3 файла, 4-й показывает пейвол-оверлей. OCR-прогон в этом окружении падает («OCR failed: unknown error», tesseract-ассеты), поэтому скачивание именно на OCR-странице проверено только типом/сборкой — путь идёт через тот же `requestDailyFileAllowance`.
- Решения, которые НЕ приняты (ждут фаундера): $0.99 day pass — рекомендация отложить (комиссия LS 5% + $0.50 съедает 44% с $0.99 против 8% с $19; продаж нет 88 дней, узкое место не в цене); Google Pay / PayPal — методы чекаута LemonSqueezy, включаются в дашборде, кода не требуют.
