...
import { resolveBillingDestination, resolveCheckoutUrl, openBillingPlans, openCheckout } from './billing';
...
describe('billing helpers', () => {
  ...
  describe('resolveCheckoutUrl', () => {
    test('returns null for empty checkout input', () => {
      assert.strictEqual(resolveCheckoutUrl('   '), null);
    });

    test('returns null for non-http checkout input', () => {
      assert.strictEqual(resolveCheckoutUrl('/checkout/buy/123'), null);
      assert.strictEqual(resolveCheckoutUrl('javascript:alert(1)'), null);
    });

    test('returns normalized absolute checkout url', () => {
      assert.strictEqual(resolveCheckoutUrl(' https://example.com/checkout '), 'https://example.com/checkout');
    });
  });
